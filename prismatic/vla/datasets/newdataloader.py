...
class _IterableDatasetFetcher(_utils.fetch._BaseDatasetFetcher):
    def __init__(self, dataset, auto_collation, collate_fn, drop_last, queue_size=10):
        ...
        self.process = multiprocessing.Process(target=self._fill_queue)
...
class _DatasetKind:
    ...
    @staticmethod
    def create_fetcher(kind, dataset, auto_collation, collate_fn, drop_last, queue_size=10):
        if kind == _DatasetKind.Map:
            return _utils.fetch._MapDatasetFetcher(dataset, auto_collation, collate_fn, drop_last)
        else:
            return _IterableDatasetFetcher(dataset, auto_collation, collate_fn, drop_last, queue_size=queue_size)
...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    def __init__(self, loader):
        ...
        self._dataset_fetcher = _DatasetKind.create_fetcher(
            self._dataset_kind, self._dataset, self._auto_collation, self._collate_fn, self._drop_last, loader.batch_size)

    def _next_data(self):
        index = self._next_index()  # may raise StopIteration
        start_time = time.time()
        data = self._dataset_fetcher.fetch(index)  # may raise StopIteration
        print(f"Time taken to fetch data: {time.time() - start_time}")
        if self._pin_memory:
