...
class RiskLearner(nn.Module):
    ...
    def __init__(self, x_dim, y_dim, r_dim, z_dim, h_dim):
        ...
        # self.prior_init_mu = torch.zeros([1, r_dim]).to('cuda')
        # self.prior_init_sigma = torch.ones([1, r_dim]).to('cuda')
        self.prior_init_mu = nn.Parameter(torch.zeros([1, r_dim]), requires_grad=False)
        self.prior_init_sigma = nn.Parameter(torch.ones([1, r_dim]), requires_grad=False)
        # self.z_prior = Normal(prior_init_mu, prior_init_sigma)

    @property
    def z_prior(self):
        return Normal(self.prior_init_mu, self.prior_init_sigma)

    def update_z_prior(self, new_mu, new_sigma):
        self.z_prior = Normal(new_mu, new_sigma)
        if dist.is_initialized():
            dist.broadcast(new_mu, src=0)
            dist.broadcast(new_sigma, src=0)
            self.z_prior = Normal(new_mu, new_sigma)
